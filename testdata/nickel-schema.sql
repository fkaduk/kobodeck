...
CREATE TABLE Rules( 
	...
	PRIMARY KEY(Id));
CREATE TABLE volume_tabs (volumeId TEXT NOT NULL,tabId TEXT NOT NULL DEFAULT 'abcdefff-ffff-ffff-ffff-fffffffffffd',PRIMARY KEY (volumeId, tabId));
CREATE TABLE ratings (ContentID TEXT NOT NULL, Rating INTEGER, Review TEXT, DateModified TEXT NOT NULL, PRIMARY KEY(ContentID));
...
CREATE TABLE user(UserID TEXT NOT NULL,UserKey TEXT NOT NULL,UserDisplayName TEXT,UserEmail TEXT,___DeviceID TEXT,FacebookAuthToken TEXT,HasMadePurchase BIT DEFAULT FALSE, IsOneStoreAccount BIT DEFAULT FALSE, IsChildAccount BIT DEFAULT FALSE, RefreshToken TEXT, AuthToken TEXT, AuthType TEXT, Loyalty BLOB, IsLibraryMigrated BIT NOT NULL DEFAULT true, SyncContinuationToken TEXT, Subscription INT NOT NULL DEFAULT 0, LibrarySyncType TEXT, LibrarySyncTime TEXT, SyncTokenAppVersion TEXT, Storefront TEXT, NewUserPromoCurrency TEXT, NewUserPromoValue REAL NOT NULL DEFAULT -1.0, KoboAccessToken TEXT, KoboAccessTokenExpiry TEXT, AnnotationsSyncToken TEXT, PrivacyPermissions BLOB, AnnotationsMigrated BIT NOT NULL DEFAULT false, NotebookSyncTime TEXT, NotebookSyncToken TEXT,PRIMARY KEY (UserID));
...
PRIMARY KEY(tabID));
CREATE TABLE OverDriveCards (CardId INTEGER NOT NULL, LibraryKey TEXT NOT NULL, BestLibraryKey TEXT NOT NULL, WebsiteId INTEGER NOT NULL, Name TEXT, LastModified TEXT, PRIMARY KEY(CardId));
...
title TEXT,
libraryKey TEXT,
cardId TEXT, isAvailable TEXT, suspensionFlag BIT, placedDate TEXT, expireDate TEXT, estimatedAvailableDate TEXT,
PRIMARY KEY(id));
...
CREATE TABLE SubscriptionProducts ( CrossRevisionId TEXT NOT NULL,Id TEXT NOT NULL,Name TEXT,IsPreOrder BOOL,Tiers TEXT,ActivationDate TEXT,DeactivationDate TEXT,PRIMARY KEY (CrossRevisionId) );
CREATE TABLE DropboxItem ( Id TEXT NOT NULL,Json TEXT,PRIMARY KEY (Id) );
CREATE TABLE KoboPlusAssetGroup ( Id TEXT NOT NULL,AssetGroup TEXT,Timestamp TEXT,Name TEXT,Url TEXT,Etag TEXT, TimestampTo TEXT, Shown BOOL DEFAULT FALSE,PRIMARY KEY (Id) );
CREATE TABLE KoboPlusAssets (AssetGroupId TEXT NOT NULL,Key TEXT NOT NULL,Language TEXT NOT NULL,Type TEXT,Value TEXT,FOREIGN KEY (AssetGroupId) REFERENCES KoboPlusAssetGroup(Id),PRIMARY KEY (AssetGroupId, Key, Language) );
CREATE TABLE GDriveItem ( Id TEXT NOT NULL,Json TEXT, Path TEXT,PRIMARY KEY (Id) );
